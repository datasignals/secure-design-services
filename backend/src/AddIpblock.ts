...
export default class AddIpblock {

    private readonly config = new ConfigCache();

    async addIpblock(ipblock_auid : string, name: string, version : string , releaseDate : string) {
        ...
        const { gasRequired, storageDeposit, result} = await contract.query.addIpblock(
            alice.address,
            {
            ...
            ipblock_auid,name,version, releaseDate);
