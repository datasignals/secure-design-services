...
export default class UpdateCurationStatus {

    private readonly config = new ConfigCache();

    async updateCurationStatus(clientAuid: string, divisionAuid : string, designerAuid: string, fileHash: string, newStatus: string) {
        ...
        const { gasRequired, storageDeposit, result} = await contract.query.updateCurationStatus(
            alice.address,
             {
             ...
             },clientAuid,divisionAuid,designerAuid,fileHash,newStatus);
