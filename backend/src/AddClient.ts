...
export default class AddClient {
    ...
    async addClient(clientAuid: string, name : string, divisions: any[]) {
        ...
        const { gasRequired, storageDeposit, result} = await contract.query.addClient(
            alice.address,
             {
