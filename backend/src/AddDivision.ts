...
export default class AddDivision {

    private readonly config = new ConfigCache();

    async addDivision(client_auid: string,divisionAuid : string, division: string, blockReleased: any[], designers: any[]) {
        ...
        const { gasRequired, storageDeposit, result} = await contract.query.addDivision(
            alice.address,
              {
              ...
              client_auid,divisionAuid,division,blockReleased, designers);
