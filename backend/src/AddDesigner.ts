...
export default class AddDesigner {
    ...
    async addDesigner(clientAuid: string, divisionAuid: string, designerAuid: string, purchase: any[], blockupload: any[]) {
        ...
        if ('maxBlock' in refTime2 && 'refTime' in (refTime2.maxBlock as any) && 'proofSize' in (refTime2.maxBlock as any)) {
          ...
          return new Promise(async (resolve, reject) => {
            console.log("inside the promsie");
          try {
          const MAX_CALL_WEIGHT = new BN(5_000_000_000_000).isub(BN_ONE);
          const { gasRequired, storageDeposit, result} = await contract.query.addDesigner(
            alice.address,
             {
              ...
              proofSize: MAX_CALL_WEIGHT
               }) as WeightV2,},clientAuid,divisionAuid, designerAuid, purchase, blockupload)
               console.log("got this fees");
                // Error handling
               if (result.isErr) {
