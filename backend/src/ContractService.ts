...
export default class ContractService {
    ...
    async addPurchase(clientAuid: string, divisionAuid: string, designerAuid: string, blockname: string, blockversion: string, time_of_purchase: string) {
        ...
        const { gasRequired, storageDeposit, result} = await contract.query.addPurchase(
            alice.address,
            {
              ...
              proofSize: MAX_CALL_WEIGHT
                ...
                clientAuid,divisionAuid,designerAuid,blockname,blockversion,time_of_purchase
        )
        ...
        return new Promise((resolve, reject) => {
         ...
         try {
            ...
            contract.tx.addPurchase({
                gasLimit: gasRequired as any as 2,
                storageDepositLimit:  storageDeposit.isCharge ? storageDeposit.asCharge.toString() : null,
             }, clientAuid,divisionAuid,designerAuid,blockname,blockversion,time_of_purchase).signAndSend(alice, res  =>  { 
                checkStatus(res);
             })
            } catch (error) {
                console.error('Error:', error);
